...
export class ScheduleCondition {
  ...
  match(date: string, time: string): boolean {
    return (
      (this.weekday === "*" || date.includes(this.weekday)) &&
...
function needAlert(from: Availability, to: Availability): boolean {
  return (
    (from.isFull() && to.isAvailable()) || (to.isFull() && from.isAvailable())
  );
}
...
export class Calendar {
  ...
  static fromTable(
    courtTable: GoogleAppsScript.XML_Service.Element,
    dates: string[],
    year: string,
    month: string
  ): Calendar {
    ...
    const scheduleNames = rows[1]
      .getChildren("td")
      .map((e) => e.getValue().trim());
    const schedules = rows.slice(2).map(
      (d, i) =>
        ({
          date: dates[i],
          schedule: d
    ...
    values.forEach((row: any[]) => {
      ...
      for (let i = 1; i < row.length; i++) {
        dayAvailability.schedule.push(
          new Availability(scheduleNames[i - 1], row[i])
        );
      }
